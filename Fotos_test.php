...
<?php
	...
	$patio= $_GET['patio'];
	 if ($patio=='CTG')
	 $sql  = "SELECT DISTINCT replace(RUTA,'/fotos_patios','') RUTA, substr(RUTA,19,11) CONTENEDOR FROM Fotos where (ID_MOVIMIENTO = ".$_GET['id']." or ID_MOVIMIENTO = nvl(".$_GET['label'].",-1) ) and PATIO = '".$_GET['patio']."'";
	 else
...
<body style="background-color:Navy" >
	...
	$patio= $_GET['patio'];
	 if ($patio=='CTG')
	 {$sql  = "SELECT DISTINCT RUTA FROM Fotos where ( ID_MOVIMIENTO = ".$_GET['id']." or ID_MOVIMIENTO = ".$_GET['label'].") and PATIO = '".$_GET['patio']."'";}
	 else{
