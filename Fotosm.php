...
<?php
	 ...
	 $sql  = "SELECT DISTINCT replace(RUTA,'/fotos_patios','') RUTA, substr(RUTA,19,11) CONTENEDOR FROM Fotos where ID_MOVIMIENTO = ".$_GET['id']." and PATIO = '".$_GET['patio']."'";
...
<section id="examplese" class="examples-sectione">
	...
	<?php
	 ...
	 $sql2  = "SELECT DISTINCT RUTA FROM Fotos where ID_MOVIMIENTO = ".$_GET['id']." and CARGO='REPARACION' and PATIO = '".$_GET['patio']."'";
...
if (!$db2 = @ocinlogon("Digitalizacion","Digitalizacion", $db2)) 
	{
    $error = ocierror();
    printf("CONNECT error: %s", $error["message"]);
    die();
	}
if (!$stmt2 = @ociparse($db2,$sql2)) 
	{
    $error = ocierror($db2);
    printf("ociparse error: %s", $error["message"]);
	} 
...
?>
	...
	<center><button onclick="downloadAll(window.links)">Descargar Fotos</button></center><BR>
</body>
    </html>
<footer>
